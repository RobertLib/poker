...
//          [<music.wav> [gain]]
//
//  Ace High ships with no audio at all — the sounds are synthesised at launch —
...
// a target — the encoder spends what the picture needs and no more. A poker table
// is a flat green oval that barely moves, so it needs almost nothing: asking for
...
func transfer(from output: AVAssetReaderOutput, to input: AVAssetWriterInput,
    ...
    let queue = DispatchQueue(label: "cz.rob.poker.appstore-conform")
